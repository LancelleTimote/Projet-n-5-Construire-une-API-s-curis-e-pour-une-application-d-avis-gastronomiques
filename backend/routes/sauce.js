...
router.post('/', (req, res, next) => {    //post pour traiter seulement les requêtes post
...
router.get('/:id', (req, res, next) => { //premier segment dynamique, le frontend va envoyer l'id de l'objet, pour pouvoir aller chercher cette id on utilise ":id"
...
router.get('/', (req, res, next) => { //le premier argument est l'URL visé par l'application (le endpoint, la route), l'url total serait http://localhost:3000/api/stuff
